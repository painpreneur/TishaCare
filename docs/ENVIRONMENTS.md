...
Логика бота сейчас продублирована между `apps/web/lib/bot.ts` и
`apps/bot/index.ts` вручную — правку нужно вносить в оба места (эпик E3
сводит их в общий модуль).
...
3. Vercel: переменные в окружении **Preview** (`APP_ENV=staging`, свой `DATABASE_URL`, токен, `WEBAPP_URL` = алиас Preview, свои секреты).
...
2. Боевой бот.
3. Vercel: переменные в окружении **Production** (`APP_ENV=production`, `MINIAPP_DEV_BYPASS` не задан).
4. `setWebhook` на боевой домен с `secret_token`.

## Чеклист выноса прод-доступа с локальной машины

- [ ] `apps/web/.env.local` — `DATABASE_URL` не боевой, `APP_ENV=local`
- [ ] `apps/bot/.env` — `DATABASE_URL` не боевой, токен dev-бота, `APP_ENV=local`
- [ ] `packages/db/.env` — `DATABASE_URL` не боевой
- [ ] боевые `DATABASE_URL` / `OPENAI_API_KEY` / `*_SECRET` — только в Vercel Production
