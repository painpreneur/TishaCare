...
// ── PHQ-9 (депрессия) ───────────────────────────────────────────────────────
// Spitzer, Williams, Kroenke / Pfizer — публично доступен, разрешение не требуется.
// Пункт 9 (мысли о самоповреждении) особый: любой ненулевой ответ — повод
// связаться с врачом, и он поднимает флаг в триаже на панели врача.
...
export const PHQ9_CODE = "PHQ9";
/** 0-indexed position of the self-harm / suicidal-ideation item. */
export const PHQ9_ITEM9_INDEX = 8;

export const PHQ9_DEF: QuestionnaireDef = {
  code: PHQ9_CODE,
  title: "PHQ-9 (шкала депрессии)",
  description: "Короткий опросник выраженности депрессии за 2 недели (9 вопросов)",
  phases: ["diagnostic", "monitoring"],
  disclaimer:
    "PHQ-9: скрининговый инструмент, а не диагноз. Обсудите результат с врачом. " +
    "Если в последнем вопросе вы отметили что-либо кроме «Совсем нет», не " +
    "оставайтесь с этим наедине: свяжитесь с врачом, а при остром состоянии — " +
    "с экстренной помощью (103) или телефоном доверия (8-800-2000-122).",
  attribution: "PHQ-9 © Pfizer Inc. (Spitzer R.L., Williams J.B.W., Kroenke K.). Разрешение на использование не требуется.",
  questions: [
    ...
    "Недовольство собой; чувство, что вы неудачник(-ца) или подвели себя или близких",
    "Трудно сосредоточиться, например на чтении или просмотре телевизора",
    "Заторможенность в движениях и речи, заметная окружающим, — или, наоборот, суетливость и неусидчивость сильнее обычного",
    "Мысли о том, что лучше было бы умереть, или о том, чтобы причинить себе вред",
  ].map((text, i) => ({ id: `q${i + 1}`, text, options: phq9Freq })),
  bands: [
    { max: 4, label: "минимальная депрессивная симптоматика", note: "Значимых признаков депрессии не выявлено." },
    { max: 9, label: "лёгкая депрессия", note: "Стоит понаблюдать за состоянием и упомянуть об этом врачу при следующем визите." },
    { max: 14, label: "умеренная депрессия", note: "Рекомендуется в ближайшее время обсудить симптомы с врачом." },
    { max: 19, label: "умеренно тяжёлая депрессия", note: "Рекомендуется как можно скорее связаться с врачом для коррекции терапии." },
    { max: 27, label: "тяжёлая депрессия", note: "Результат указывает на тяжёлое состояние. Свяжитесь с врачом как можно скорее." },
  ],
};

// ── YMRS (шкала мании Янга) — адаптированная самооценка ──────────────────────
// Оригинал заполняет врач по клиническому интервью. Здесь — самооценка для
// отслеживания динамики между визитами. 4 пункта (раздражительность, речь,
// содержание мыслей, агрессивное поведение) весят вдвое — как в оригинале,
// поэтому у них шаг значений 0-2-4-6-8, максимум шкалы 60.

const ymrsX1 = (anchors: [string, string, string, string, string]) =>
  anchors.map((label, i) => ({ label, value: i }));
const ymrsX2 = (anchors: [string, string, string, string, string]) =>
  anchors.map((label, i) => ({ label, value: i * 2 }));

export const YMRS_CODE = "YMRS";
export const YMRS_DEF: QuestionnaireDef = {
  code: YMRS_CODE,
  title: "YMRS (шкала мании), самооценка",
  description: "Отслеживание выраженности подъёма настроения за последнюю неделю (11 пунктов)",
  phases: ["monitoring"],
  disclaimer:
    "Оригинальную шкалу Янга (YMRS) заполняет врач по итогам клинического " +
    "интервью и наблюдения. Это адаптированная самооценка: она помогает видеть " +
    "динамику между визитами, но не является клинической оценкой и не заменяет " +
    "её. Обязательно обсудите результат с врачом, особенно при высоком балле.",
  attribution: "По мотивам Young Mania Rating Scale (Young R.C. et al., 1978). Адаптированная самооценка.",
  questions: ([
    {
      text: "Приподнятое настроение за последнюю неделю",
      options: ymrsX1([
        "Обычное для меня",
        "Слегка приподнятое, если о чём-то спрашивают",
        "Явно приподнятое, чувствую оптимизм",
        "Приподнятое и неуместно бодрое, шучу больше обычного",
        "Эйфория, неуместный смех",
      ]),
    },
    {
      text: "Двигательная активность и энергия",
      options: ymrsX1([
        "Как обычно",
        "По ощущениям энергии больше",
        "Оживлён(а), жестикуляции больше",
        "Избыток энергии, временами трудно усидеть",
        "Почти всё время в движении, усидеть не могу",
      ]),
    },
    {
      ...
      options: ymrsX1([
        "Обычный",
        "Немного повышен",
        "Заметно повышен по ощущениям",
        "Явно повышен, говорю об этом спонтанно",
        "Поглощает мысли и поступки",
      ]),
    },
    {
      text: "Сон",
      options: ymrsX1([
        "Сплю как обычно",
        "Сплю примерно на час меньше обычного",
        "Сплю более чем на час меньше",
        "Потребность во сне снизилась, сплю мало и не устаю",
        "Почти не сплю и не чувствую усталости",
      ]),
    },
    {
      ...
      options: ymrsX2([
        "Обычная",
        "Временами повышена",
        "Часто раздражён(а) и резок(-ка) в течение дня",
        "Раздражён(а) почти всё время, вспыльчив(-а)",
        "Постоянно на пределе, конфликты почти со всеми",
      ]),
    },
    {
      text: "Речь: темп и количество",
      options: ymrsX2([
        "Как обычно",
        "Говорю больше обычного",
        "Временами говорю много и быстро",
        "Часто говорю напористо, меня трудно перебить",
        "Говорю почти безостановочно, перебить невозможно",
      ]),
    },
    {
      text: "Скорость и связность мыслей",
      options: ymrsX1([
        "Как обычно",
        "Мысли скачут, легко отвлекаюсь",
        "Перескакиваю с темы на тему, за мыслью трудно уследить",
        "Мысли летят, теряю нить, окружающим сложно понять",
        "Речь бессвязная, понять почти невозможно",
      ]),
    },
    {
      text: "Содержание мыслей",
      options: ymrsX2([
        "Обычное",
        "Много новых планов и интересов",
        "Особые проекты, повышенная значимость происходящего",
        "Идеи собственного величия или особой миссии",
        "Убеждённость в сверхспособностях, видения или голоса",
      ]),
    },
    {
      text: "Раздражающее или агрессивное поведение",
      options: ymrsX2([
        "Нет",
        "Саркастичен(-чна), временами резок(-ка)",
        "Открытые вспышки гнева, повышаю голос",
        "Угрозы, конфликты, ломаю вещи",
        "Физическая агрессия, драки",
      ]),
    },
    {
      text: "Внешний вид и уход за собой",
      options: ymrsX1([
        "Как обычно",
        "Немного небрежнее обычного",
        "Одет(а) и причёсан(а) кое-как, слежу за собой хуже",
        "Выгляжу растрёпанно, одеваюсь ярко или необычно",
        "Совсем не слежу за собой, вычурная одежда или макияж",
      ]),
    },
    {
      text: "Насколько вы согласны, что ваше состояние изменилось",
      options: ymrsX1([
        "Вижу изменения и признаю их",
        "Допускаю, что, возможно, взвинчен(а)",
        "Замечаю перемены в настроении, но проблемой не считаю",
        "Скорее отрицаю какие-либо изменения",
        "Полностью уверен(а), что со мной всё в порядке",
      ]),
    },
  ] as { text: string; options: QDefOption[] }[]).map((q, i) => ({ id: `q${i + 1}`, ...q })),
  bands: [
    { max: 7, label: "без признаков подъёма", note: "Выраженных признаков маниакального или гипоманиакального подъёма по самооценке не выявлено." },
    { max: 14, label: "слабо выраженный подъём", note: "Возможен гипоманиакальный сдвиг. Стоит рассказать об этом врачу в ближайшее время." },
    { max: 25, label: "умеренно выраженный подъём", note: "Рекомендуется как можно скорее связаться с врачом." },
    { max: 60, label: "выраженный подъём", note: "Результат указывает на выраженное состояние. Свяжитесь с врачом безотлагательно." },
  ],
};
...
export const QUESTIONNAIRE_DEFS: Record<string, QuestionnaireDef> = {
  ...
  [PHQ9_DEF.code]: PHQ9_DEF,
  [YMRS_DEF.code]: YMRS_DEF,
};
